...
# {state_name: {reply_keyword: (next_state, next_err, [utterances])}
Dialogue_dict = {
    ...
    'qotd0': {'NA': ('qotd1', 'err_askhelp', ['Cool, thanks for sharing with me. Today we are going to continue the story with our superhero Zip and see how he can help another child.',
                    "Before we start our new journey. I want to ask if either of you have asked for help before?"])},
    'qotd1': {'yes': ('story0', '', ['Great. Can one of you share when you asked for help and how it helped you to solve your problem?'])},
    'story0': {'NA': ("story1", 'err_task1', ['Thanks for sharing! I\'m sure many kids have similar stories like yours!', 
                    'Today we are going to read how Zip helped another child to ask for help. Now let\'s move on to our story.',
                    'One evening, Zip got an alert that a child needed help. He needed to sneak out of his house carefully without his family finding out about his secret superhero identity...',
                    'Now it\'s your turn to be creative! Do you think Zip can sneak out of his house without his family noticing? \n A. Of course he can! He\'s a superhero! \n B. No, I think his parents will hear him leaving. \n Please say "A" or "B" to answer!'])},
    ...
    'takehome': {'NA': ('', '', ['Thanks for sharing! How do you feel when you hearing the other\'s request? Think about this question, and I will ask one of you to share with me during tomorrow\'s story time.',
                    'Thanks for attending today\'s story and learning a new asking for help skill. Remember to come back tomorrow and I will tell you another story of our superhero Zip. See you tomorrow!'])}
}
# 'sharing': {"NA": ("qotd1", [""])},
...
Answer_keys = {
    ...
    'no': ['no', 'nope', 'not'],
    'abc': ['a', 'at', 'hey', 'b', 'be', 'bean', 'bee', 'beam', 'pee', 'p', 'c', 'see', 'sea'],
    'a': ['a', 'at', 'hey'],
    'b': ['b', 'bee', 'be', 'bean', 'beam', 'p', 'pee'],
    'c': ['c', 'sea', 'see'],
}

Error_response = {
   'err_exit': (['Okay, seems like you need more time. Come back soon, whenever you are ready to start!'], ''),
   'err_askhelp': (['Today we are going to read how Zip help another child to ask for help. Now let\'s move on to our story.',
                    'One evening, Zip got an alert that a child needed help. He needed to sneak out of his house carefully without his family finding out about his secret superhero identity...',
                    'Now it\'s your turn to be creative! Do you think Zip can sneak out of his house without his family noticing? \n A. Of course he can! He\'s a superhero! \n B. No, I think his parents will hear him leaving. \n Please say "A" or "B" to answer!'], 'story1', 'err_task1'),
    'err_task1': (['Hmm... Sorry I couldn\'t understand that. Please say "A" or "B" to answer.'], 'story1'),
    'err_task3': (['Hmm... Sorry I couldn\'t understand that. Please say "A", "B", or "C" to answer.'], 'story3'),
    'err_learning': (['Hmm... Sorry I couldn\'t understand that. Please say "Yes" or "No" to answer.'], 'recap1')
}
...
class dialogueMaintainer(object):
    ...
    def getDialogue(self, userinput):
        ...
        if len(answer_key_word) == 0:
            resp = Error_response[self.error][0]

            if len(Error_response[self.error]) > 1:
                self.status = Error_response[self.error][1]
            if len(Error_response[self.error]) > 2:
                self.error = Error_response[self.error][2]

            return resp
        
        next_state, error, responses = Dialogue_dict[self.status][answer_key_word]
        self.status = next_state
        self.error = error
        self.lastresponse = responses
        print("state is " + self.status + " and err handling is " + self.error)
        return responses
