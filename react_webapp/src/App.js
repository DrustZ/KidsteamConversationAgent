...
var silentTime = 6000; // 8 secs
var timeInterval_short;
var timeInterval_long;
var currentSilence = 0; // max silences = 2
...
function App() {
  ...
  const [recognizeFinished, setRecognizeFinished, recognizeFinishedRef] =
    useState(true);
  // whether the Mic button is clicked or not
  ...
  const initTimer = () => {
    ...
    timeInterval_short = setTimeout(function () {
      if (currentSilence === 0) {
        ...
        socket.emit("speechText", {
          text: "If you finish, please click the button.",
        });
        currentSilence += 1;
        pauseRecording();
      }
